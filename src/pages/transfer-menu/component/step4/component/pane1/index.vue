...
// 列表数据
const NewlyAddedData = reactive<{value: DictCode[]}>({value: []})
 ...
 * 新增数据字典到正式环境
 */
const addDictCode = async () =>{
  if(!dictCodeStore.dictCodesUpdatable.length) {
    ElMessage.info("请选择需要新增到正式环境的数据字典！")
    return
  }
  await dictCodeStore.beforeUpdateDictCodes('add')
  await getData()
}
...
const initValue = (data: DictCode[]) => {
  NewlyAddedData.value = timeSort(data, 'modifyTime')
...
const filterListData = () => {
  let filterData = dictCodeStore.newlyAddedDictCodeData
...
const getData = async () => {
  ...
  try{
    ...
    initValue(dictCodeStore.newlyAddedDictCodeData)
  } finally {
...
onMounted(async () => {
  if(dictCodeStore.newlyAddedDictCodeData.length) return;
  await getData()
})
...
<template>
  <div class="left-content" v-loading="transferMenuGlobalStore.loading">
    ...
    <div style="margin: 0 0 10px;text-align: left">
      <el-button type="primary" @click="addDictCode">新增</el-button>
    </div>
    <el-table
